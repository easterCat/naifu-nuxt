        ...
        <el-row :gutter="20">
            ...
            <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">
                ...
                >
                    <template #prepend>
                        <el-select
                            v-model="postUrlSelect"
                            placeholder="选择接口"
                            style="width: 115px"
                            size="large"
                            @change="getPostUrlChange"
                        >
                            <el-option
                                label="Colab"
...
const postUrlSelect: Ref<string> = ref('');
...
const getPostUrlChange = (value: string) => {
    if (value) {
        postUrlSelect.value = value;
        postUrl.value = value;
    }
};
    ...
    try {
        if (generateNum.value === 1) {
            ...
            const results = await Promise.all(
                Array.from({ length: generateNum.value }).map((i) => {
                    const seed = Math.floor(Math.random() * Math.pow(2, 32));
